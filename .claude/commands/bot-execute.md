...
6. After a confirmed fill, record it: update `trading-bot/data/portfolio.json` — move the ticket into positions with the actual fill price (mirror what `executeTicketPaper` in trading-bot/src/paper.js does, using real fill values), and mark the ticket status `executed_live`.
